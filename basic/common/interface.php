...
function sendQueue($host, $port, $type, $msg) {
	...
	$ret = $request->connect();
	if($ret === false){
           $cwx_error = $request->getLastError();
	   echo $cwx_error;
           exit;
	}
...
function getTaskScriptList($host, $port, $prefix, $num = 10) {
  $ui_script = new Dcmd_api_UiScripts();
  $ui_script->setClientMsgId(0);
  $ui_script->setScriptType(1);
  $ui_script->setScriptPrefix($prefix);
  $ui_script->setFetchNum($num);
  $ui_script->setUser(CENTER_USER);
  $ui_script->setPasswd(CENTER_PASSWD);
  $msg = $ui_script->serializeToString();
  $ret_msg = sendQueue($host, $port, Dcmd_api_DcmdMsgType::MTYPE_UI_SCRIPTS, $msg); 
  $ui_script_reply = new Dcmd_api_UiScriptsReply();
  try{
    $ui_script_reply->parseFromString($ret_msg);
  }catch(Exception $ex) {
    die("Parse err:".$ex->getMessage());
  }
  return $ui_script_reply;
}

function getOprScriptList($host, $port, $prefix, $num = 10) {
  $ui_script = new Dcmd_api_UiScripts();
  $ui_script->setClientMsgId(0);
  $ui_script->setScriptType(2);
  $ui_script->setScriptPrefix($prefix);
  $ui_script->setFetchNum($num);
  $ui_script->setUser(CENTER_USER);
  $ui_script->setPasswd(CENTER_PASSWD);
...
function getRunningTask($host, $port, $ip, $svr_pool = NULL, $app_name=NULL, $svr_name=NULL) {
  ...
  if($svr_pool) $ui_agent_running_task->setSvrPool($svr_pool);
  if($app_name) $ui_agent_running_task->setAppName($app_name);
  if($svr_name) $ui_agent_running_task->setSvrName($svr_name);
...
function execRepeatOprCmd($host, $port, $repeat_opr_name, $args, $agents) {
  $repeat_exec_opr_cmd = new Dcmd_api_UiExecDupOprCmd ();
  $repeat_exec_opr_cmd->setClientMsgId(0);
  $repeat_exec_opr_cmd->setDupOprName($repeat_opr_name);
  foreach($args as $k=>$v) {
    $kv = new Dcmd_api_KeyValue();
    $kv->setKey($k);
    $kv->setValue($v);
    $repeat_exec_opr_cmd->appendArgs($kv);
  }
  foreach($agents as $ip) $repeat_exec_opr_cmd->appendAgents($ip);
  $repeat_exec_opr_cmd->setUser(CENTER_USER);
  $repeat_exec_opr_cmd->setPasswd(CENTER_PASSWD);
  $msg = $repeat_exec_opr_cmd->serializeToString();
  $ret_msg = sendQueue($host, $port, Dcmd_api_DcmdMsgType::MTYPE_UI_EXEC_DUP_OPR , $msg);
  $repeat_exec_opr_cmd_reply = new Dcmd_api_UiExecDupOprCmdReply();
  try{
   $repeat_exec_opr_cmd_reply->parseFromString($ret_msg);
  }catch(Exception $ex) {
    die("Parse err:".$ex->getMessage());
  }
  return $repeat_exec_opr_cmd_reply;
}

...
<div class="dcmd-task-template-form">
    ...
    <?= $form->field($model, 'app_id')->dropDownList($app, ['onchange'=>'javascript:getService()', $model->app_id ? "disabled" : "" =>true])->label('产品名称') ?>

    <?= $form->field($model, 'svr_id')->dropDownList($svr, ['onchange'=>'javascript:setSvr()', $model->svr_id ? "disabled" : "" =>true])->label('服务名称') ?>
  
    <?= $form->field($model, 'task_cmd_id')->dropDownList($task_cmd, ["onchange"=>"javascript:taskTypeSelect()", $model->task_cmd_id ? "disabled" : "" =>true])->label('任务脚本') ?>
